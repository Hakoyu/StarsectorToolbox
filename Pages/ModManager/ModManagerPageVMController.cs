...
namespace StarsectorTools.Pages.ModManager
{
    internal partial class ModManagerPageViewModel
    {
        ...
        private void InitializeData()
        {
            ...
            ModsInfo.AllUserGroups = allUserGroups.AsReadOnly<string, HashSet<string>, IReadOnlySet<string>>();
            GetAllModsInfo();
            GetAllListBoxItems();
            GetTypeGroup();
            GetAllModsShowInfo();
        ...
        private void GetAllModsInfo()
        {
            ...
            Logger.Record(string.Format(I18nRes.ModAddCompleted, allModInfos.Count, errSize), LogLevel.INFO);
        ...
        {
            var result = MessageBoxVM.Show(new(I18nRes.SelectImportMode) { Button = MessageBoxVM.Button.YesNoCancel, Icon = MessageBoxVM.Icon.Question });
        ...
        private void RefreshModsContextMenu()
        {
            ...
            Logger.Record($"{I18nRes.ContextMenuRefreshCompleted} {I18nRes.Size}: {allModsShowInfo.Values.Count}");
        }
        ...
        private ContextMenuVM CreateModShowContextMenu(ModShowInfo showInfo)
        {
            ...
            ContextMenuVM contextMenu = new((list) =>
            {
                // 启用或禁用
                ...
                {
                    Logger.Record($"{I18nRes.OpenModDirectory} {I18nRes.Path}: {allModInfos[showInfo.Id].ModDirectory}");
                ...
                menuItem.CommandEvent += (p) =>
                {
                    ...
                    if (MessageBoxVM.Show(new($"{I18nRes.ConfirmModDeletion}?\nID: {showInfo.Id}\n{I18nRes.Path}: {path}\n") { Button = MessageBoxVM.Button.YesNo, Icon = MessageBoxVM.Icon.Warning }) is MessageBoxVM.Result.Yes)
                    {
                        Logger.Record($"{I18nRes.ConfirmModDeletion}?\nID: {showInfo.Id}\n{I18nRes.Path}: {path}\n");
        ...
        private void ChangeModEnabled(string id, bool? enabled = null)
        {
            ...
            Logger.Record($"{id} {I18nRes.ChangeEnabledStateTo} {showInfo.IsEnabled}", LogLevel.DEBUG);
        }
        ...
        private void CheckEnabledModsDependencies()
        {
            foreach (var showInfo in allModShowInfoGroups[ModTypeGroup.Enabled])
            {
                if (showInfo.DependenciesSet != null)
                {
                    ...
                    {
                        Logger.Record($"{showInfo.Id} {I18nRes.NotEnableDependencies} {showInfo.Dependencies}");
        ...
        private void ChangeModCollected(string id, bool? collected = null)
        {
            ...
            Logger.Record($"{id} {I18nRes.ChangeCollectStateTo} {showInfo.IsCollected}", LogLevel.DEBUG);
        }
            ...
            if (filesInfo.FirstOrDefault(defaultValue: null) is FileInfo fileInfo && fileInfo.FullName is string jsonPath)
            {
                ...
                if (allModInfos.ContainsKey(newModInfo.Id))
                {
                    ...
                    if (result == MessageBoxVM.Result.Yes)
                    {
                        ...
                        RefreshCountOfListBoxItems();
                        StartRemindSaveThread();
                        Logger.Record($"{I18nRes.ReplaceMod} {newModInfo.Id} {originalModInfo.Version} => {newModInfo.Version}");
                    }
        ...
        {
            if (MessageBoxVM.Show(new(I18nRes.ConfirmUserGroupDeletion) { Button = MessageBoxVM.Button.YesNo, Icon = MessageBoxVM.Icon.Question }) is MessageBoxVM.Result.No)
                return;
            ...
            {
                if (ComboBox_ExportUserGroup[i] is ComboBoxItemVM comboBoxItem && comboBoxItem.Content.ToString()! == name)
                {
                    // 如果此选项正被选中,则选定到All
